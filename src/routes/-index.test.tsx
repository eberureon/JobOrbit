...
describe("Dashboard", () => {
	...
	it("renders stat card labels", async () => {
		render(<DashboardPage />, { wrapper: Wrapper });
		expect(
			(await screen.findAllByText("Total Applications")).length,
		).toBeGreaterThanOrEqual(1);
		expect(
			(await screen.findAllByText("Last 7 Days")).length,
		).toBeGreaterThanOrEqual(1);
		expect(
			(await screen.findAllByText("Last 30 Days")).length,
		).toBeGreaterThanOrEqual(1);
		expect(
			(await screen.findAllByText("Active Pipeline")).length,
		).toBeGreaterThanOrEqual(1);
	});
	...
	it("renders the funnel section", async () => {
		render(<DashboardPage />, { wrapper: Wrapper });
		expect(
			(await screen.findAllByText("Success Rate Funnel")).length,
		).toBeGreaterThanOrEqual(1);
	});
	...
	it("renders recent applications when data exists", async () => {
		...
		render(<DashboardPage />, { wrapper: Wrapper });
		expect(
			(await screen.findAllByText("NewCorp")).length,
		).toBeGreaterThanOrEqual(1);
	});
});
